...
#' @export
#'
refit_lambda <- function(sym_ebcovmf_obj, maxiter = 100, tol = 10^(-6), remove_null = TRUE){
  ...
  if (K <= 1){
    ...
    while((iter <= maxiter) && (obj_diff > tol)){
      ...
      for (k in 1:K){
        Rk <- sym_ebcovmf_obj$S - tcrossprod(sym_ebcovmf_obj$L_pm[,-k] %*% diag(sqrt(sym_ebcovmf_obj$lambda[-k]), ncol = (K-1)))

...
#' @export
#'
sym_ebcovmf_backfit <- function(sym_ebcovmf_obj, ebnm_fn, backfit_maxiter = 100, backfit_tol = 10^(-8), optim_maxiter= 500, optim_tol = 10^(-8)){
  ...
  while((iter <= backfit_maxiter) && (obj_diff > backfit_tol)){
    ...
    for (k in 1:K){
      ...
      R <- sym_ebcovmf_obj$S - tcrossprod(sym_ebcovmf_obj$L_pm[,-k, drop = FALSE] %*% diag(sqrt(sym_ebcovmf_obj$lambda[-k]), ncol = (K-1)))
      R2k <- compute_R2(sym_ebcovmf_obj$S,
                        sym_ebcovmf_obj$L_pm[,-k, drop = FALSE],
                        sym_ebcovmf_obj$lambda[-k], (K-1))

      # optimize factor
      factor_proposed <- optimize_factor(R,
                                         ebnm_fn,
                                         optim_maxiter,
                                         optim_tol,
                                         sym_ebcovmf_obj$L_pm[,k],
                                         sym_ebcovmf_obj$lambda[k],
                                         sym_ebcovmf_obj$fitted_gs[[k]],
                                         R2k,
                                         sym_ebcovmf_obj$n,
                                         sym_ebcovmf_obj$KL[-k])
